...
for(B1 in sort(unique(data11$Brand))){
  ...
  # model_train <- forecast(hts_train,h=forecastperiod,method="bu")
  # 
  ...
  # MAPA with HTS
  ally <- aggts(htsobject)
  allf <- matrix(NA, nrow = forecastperiod, ncol = ncol(ally))
  colnames(allf) <- colnames(ally)
  for(i in sort(unique(colnames(ally)))){
    allf[,i] <- mapafor(ally[,i],
                        mapaest(ally[,i], outplot=0),
                        fh = forecastperiod,
                        outplot=0)$outfor
    
    #allf[,i] <- ses((ally[,i]), h = forecastperiod)$mean
  }
  allf <- ts(allf)
  y.f <- combinef(allf, htsobject$nodes, keep = "bottom")
  rr <- (sum(t1$`n_distinct(SubBrand)`)+1)+1
  tf <- ally[,rr:ncol(ally)]
  colnames(y.f) <- colnames(tf)
  #print(y.f[1,])
  model_dcastData <- y.f
  #model_dcastData <- forecast(htsobject, h=forecastperiod, method="bu",fmethod="arima")
  model_dcastData <- as.data.frame(model_dcastData)
  model_dcastData$Date <- seq(as.Date(add.month(maxdate,1)), as.Date(add.month(maxdate,forecastperiod)), by="month")
  
  bottomLevelForecast <- reshape2::melt(model_dcastData, id.vars = "Date")
  bottomLevelForecast <- rename(bottomLevelForecast, Forecast = value, SKU=variable)
  bottomLevelForecast$Method <- "MAPA_HTS"
  ...
  finalForecast <- rbind(finalForecast,bottomLevelForecast)
